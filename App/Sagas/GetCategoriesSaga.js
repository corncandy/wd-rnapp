...
export default (api) => {
  ...
  function * worker (shop) {
    // make the call to the api
    const response = yield call(api.getCategories, shop)

    // success?
    if (response.ok) {
      yield put(Actions.receiveCategories(response.data))
    } else {
      yield put(Actions.receiveCategoriesFailure())
    }
  }
  ...
  function * watcher () {
    while (true) {
      const action = yield take(Types.CATEGORIES_REQUEST)
      const { shop } = action
      yield call(worker, shop)
    }
  }
